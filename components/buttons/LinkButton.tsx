...
export default function LinkButton({ target = "home", color = "colored", eventId }: LinkButtonProps) {
  ...
  const colorClass = color === "white" ? "white-button" : "colored-button";

  const positionClass = ["client", "home", "client-profile", "event"].includes(target) ? "md:left-20" : "md:right-20"; 

  return (
    <button type="button" onClick={handleClick} className={`link-button ${colorClass} ${positionClass}`}>
      {getLabel()}
    </button>
  );
};